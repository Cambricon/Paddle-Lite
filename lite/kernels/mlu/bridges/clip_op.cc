...
int ClipConverter(void* ctx, OpLite* op, KernelBase* kernel) {
  ...
  // attr
  auto min = op_info->GetAttr<float>("min");
  auto max = op_info->GetAttr<float>("max");
  ...
  cnmlBaseOp_t clip_op;
  CNML_CALL(cnmlCreateClipOp(&clip_op,
                            input_tensor->mlu_tensor(),
                            output_tensor->mlu_tensor(),
                            min,
                            max));

  graph->FuseOp(clip_op);
  CNML_CALL(cnmlDestroyBaseOp(&clip_op));
  return SUCCESS;
}
...
REGISTER_SUBGRAPH_BRIDGE(clip,
                         kMLU,
                         paddle::lite::subgraph::mlu::ClipConverter);
